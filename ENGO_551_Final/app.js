...
map.on('style.load', function () {
    ...
    map.loadImage(
        ...
        function (error, image) {
            ...
            // Update asset positions along nearby roads every second
            setInterval(() => {
                assets.forEach(async asset => {
                    // Generate a random destination nearby
                    const destLng = asset.coordinates[0] + (Math.random() - 0.5) * 0.02;
                    const destLat = asset.coordinates[1] + (Math.random() - 0.5) * 0.02;
                    
                    // Get route from current position to destination
                    const response = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${asset.coordinates[0]},${asset.coordinates[1]};${destLng},${destLat}?steps=true&geometries=geojson&access_token=${mapboxgl.accessToken}`);
